...
terapias = [
  ...
  {
    "nomeTerapia": "Drenagem Linfática",
...
for terapia in terapias:
    try:
        response = requests.post(BASE_URL, json=terapia)
        if response.status_code == 201:
            print(f"✅ Terapia '{terapia['nomeTerapia']}' criada com sucesso!")
        else:
            print(f"❌ Erro ao criar '{terapia['nomeTerapia']}': {response.text}")
    except Exception as e:
        print(f"❌ Erro de conexão ao tentar criar '{terapia['nomeTerapia']}': {str(e)}")